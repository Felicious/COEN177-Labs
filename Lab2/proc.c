 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {
	int lottery = rand() % (10 + 1 - 1); /* generates a random # from 0 - 9 */
	int r = rand() % (NR_SCHED_QUEUES + 1 - 3); /*randomly assigns lower queue priority between 16 and 3 */ 
	if(lottery == 3) /*10% of time, (when 3 is selected out of 3-9 ,*/
		q = r; 	/*the process is assigned to the random lower priority queue*/
	if(!(rp = rdy_head[q])) {
